...
description: 这篇文章将介绍什么是astro的catch-all路由, 以及它的用途和实现。
...
将是一个字符串数组，其中每个元素代表 URL 路径的一个段（不包括开头的 / 和末尾的 /）。

补充说明： 对于 [...slug] 这样的剩余参数，Astro.params.slug
...
slug.join('/')。 3. 分页 () 中的 Catch-All 路由 paginate() 函数也可以与 catch-all 路由结合使用，例如
