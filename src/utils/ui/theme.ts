...
import type { Mode, Theme, ThemeConfig } from "@/constants/theme";
...
export const getCurrentTheme = () => {
  ...
  const isValidThemeName =
    Boolean(themeName) &&
    THEMES.map((theme) => theme.name).includes(themeName as Theme["name"]);
...
/**
 * 根据在客户端配置中设置的单一默认主题（一个模式及其对应的主题名称），
...
export const getDefaultThemes = () => {
  ...
  const otherTheme = DEFAULT_THEME.replace(
    DEFAULT_MODE,
    otherMode
  ) as Theme["name"];
